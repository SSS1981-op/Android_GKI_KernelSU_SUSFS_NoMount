...
python3 - ".repo/manifests/default.xml" "$superproject_commit" <<'PY'
import sys
...
superprojects[0].set("revision", commit)
tree.write(path, encoding="UTF-8", xml_declaration=True)
PY

"$repo_root/repo" sync -c --no-clone-bundle --no-tags --optimized-fetch --use-superproject -j "${REPO_JOBS:-2}"
